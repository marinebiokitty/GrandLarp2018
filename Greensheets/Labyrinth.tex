...
\begin{itemize}
\item If the party succeeds on the Physical check, one party member may take a black rose petal and consume it immediately.  One participant in the Physical check must take one magical damage.
\item If the party fails on the Physical check, they must distribute two magical damage amongst themselves.
...
\item Any party member that fails the Wyrd check takes one magical damage.
\end{itemize}
The effects of eating a petal are as follows:
\begin{itemize}
\item If you have already eaten a petal today, there is no effect.
\item If your Wyrd is equal to 1, 2, or 7, your Psi score is reset to zero.
