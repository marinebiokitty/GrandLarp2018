...
\NEW{Sign}{\sTrodUse}{
  ...
  \s\MYtext	{	\emph{If you want to use the Trod, you may do so, and step across from the human world to the hedge, or from the hedge to the human world. You can't see or hear things in the other world, even though they occupy the same space. There are no mechanics for the hedge, so you are encouraged to roleplay accordingly.}
	}
}

\NEW{Sign}{\sTrodUseHedge}{
  \s\MYname	{Using The Trod} 
  \s\MYtext	{	\emph{If you want to try to use the Trod, refer to the greensheet in the attached envelope. {\bf Please leave the greensheet here for the next person to use.}}
	}
}